...
OPTIONAL_DATASETS = [
    ...
    },
    {
        ...
        "category": "health",
        "format": "openaq",
        "columns": ["location", "city", "country", "parameter", "value", "unit", "date"],
        "date_column": "date",
        "geo_column": "city",
    ...
    if not data:
        ...
        for idx in range(limit):
            ...
            for col in columns:
                ...
                elif any(token in col_lower for token in ["rate", "value", "count", "total", "cases", "index", "pct", "percent"]):
...
def fetch_dataset_data(
    ...
    if not data:
        logger.warning(f"Dataset fetch returned empty for {dataset_id}, using mock data")
        data = generate_mock_data(dataset, limit)
