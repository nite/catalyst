...
def detect_column_type(series: pd.Series, col_name: str) -> Dict[str, Any]:
    ...
    if series.dtype in ["int64", "float64"]:
        ...
        if is_date or "date" in col_name.lower() or "time" in col_name.lower():
            ...
            col_info["unique_count"] < len(series) * 0.5
            and col_info["unique_count"] < 50
        ):
...
def suggest_chart_types(
    columns: List[Dict[str, Any]], dataset_meta: Dict
) -> List[Dict[str, Any]]:
    """
...
def generate_filter_config(columns: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for col in columns:
        filter_config = {"column": col["name"], "type": col["type"]}
        ...
        if col["type"] == "temporal":
            ...
            filter_config["max"] = col.get("max")
            filter_config["step"] = (
                (col.get("max", 100) - col.get("min", 0)) / 100
                if col.get("max") and col.get("min")
                else 1
            )

        elif col["type"] == "categorical":
            filter_config["filter_type"] = "multi_select"
            filter_config["options"] = list(col.get("categories", {}).keys())[
                :20
            ]  # Limit to 20 options
...
def analyze_dataset(
    data_response: Dict[str, Any], dataset_meta: Dict
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        stats = {
            ...
            "numerical_columns": len(
                [c for c in column_analysis if c["type"] == "numerical"]
            ),
            "categorical_columns": len(
                [c for c in column_analysis if c["type"] == "categorical"]
            ),
            "temporal_columns": len(
                [c for c in column_analysis if c["type"] == "temporal"]
            ),
            "has_time_series": any(c["type"] == "temporal" for c in column_analysis),
            "has_geographic": any(
                c.get("is_geographic", False) for c in column_analysis
            ),
        }
